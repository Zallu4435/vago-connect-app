...
function DocumentMessage({ message }) {
  ...
  return (
    <>
      <div className={`flex flex-col w-full max-w-[75%] sm:max-w-[65%] gap-1 ${isIncoming ? 'items-start' : 'items-end'}`}>
        {message.isForwarded && (
          <span className="text-[10px] text-ancient-text-muted italic px-2">Forwarded</span>
        )}
        
        <div className={`message-bubble message-bubble-document ${isIncoming ? 'message-bubble-incoming' : 'message-bubble-outgoing'}`}>
          <div
            className={`relative p-3 sm:p-4 rounded-xl shadow-lg flex items-center gap-3 cursor-pointer transition-all duration-200 hover:shadow-xl group ${
              isIncoming
          ...
          >
            ...
            <div className="relative flex-shrink-0">
              <div className={`flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-lg transition-colors ${
                isIncoming 
                  ? "bg-ancient-input-bg border border-ancient-border-stone" 
                  : "bg-ancient-bg-dark/40 border border-ancient-icon-glow/40"
              }`}>
                <MdInsertDriveFile className={`text-2xl sm:text-3xl ${
                  isIncoming ? "text-ancient-icon-glow" : "text-ancient-icon-glow"
                }`} />
              </div>
              {fileExt && (
                <span className={`absolute -bottom-1 -right-1 text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded ${
                  isIncoming 
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-sm sm:text-base font-medium truncate leading-tight ${
                isIncoming ? "text-ancient-text-light" : "text-ancient-text-light"
              }`}>
                {fileName}
              </div>
            ...
            <a
              ...
              className={`flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 ${
                isIncoming
